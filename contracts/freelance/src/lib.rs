...
use soroban_sdk::{contract, contractimpl, contracttype, token, vec, Env, Val, Vec};
mod events;
...
impl IntoVal<Env, Val> for State {
    fn into_val(&self, env: &Env) -> Val {
        let numeric_value: u32 = /* conversion logic here */;
        numeric_value.into_val(env)
    }
}
...
impl Work {
    pub fn transaction(e: Env, freelancer: Address, deadline: u64, token: Address, rating: u64) {
        ...
        e.storage()
            .instance()
            .set(&DataPoint::Started, &get_ledger_timestamp(&e));
